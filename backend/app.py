...
def predict():
    try:
        ...
        cursor = db.cursor()
        sql = "INSERT INTO detections (filename, detected_at, result_text, detection_info) VALUES (%s, %s, %s, %s)"
        val = (
            filename,
...
def last_prediction():
    ...
    cursor.execute("SELECT * FROM detections ORDER BY id DESC LIMIT 1")
    row = cursor.fetchone()
    cursor.close()
    if not row:
        return jsonify({})
...
def reset_prediction():
    ...
    if row:
        ...
        cursor2 = db.cursor()
        cursor2.execute("DELETE FROM detections WHERE id = %s", (row['id'],))
        db.commit()
        cursor2.close()
    cursor.close()
